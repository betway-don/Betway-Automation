...
test.describe('Betslip Module - ZA', () => {
  ...
  test('T13-Verify functionality of "Close" button of leg on Betslip inside single and multi section', async ({ betslipPage, page }, testInfo) => {
    await OddsSelection(1, page);
    ...
    await betslipPage.clickMultiTab();
    await betslipPage.verifyRemoveLegIcon();
    await ScreenshotHelper(page, screenshotDir, 'T13-betslip-remove-leg-icon-multi.png', testInfo);
    await betslipPage.clickRemoveLegIcon();
    await betslipPage.clickBetslipButton();
    await ScreenshotHelper(page, screenshotDir, 'T13-betslip-remove-leg-after-clickmulti.png', testInfo);
  });

  test('T14-Verify functionality of Wager amount text box inside single and multi section of the betslip', async ({ betslipPage, page }, testInfo) => {
    await OddsSelection(1, page);
    ...
    await betslipPage.verifyBetAmountInput();
    await betslipPage.enterBetAmount('10');
    await ScreenshotHelper(page, screenshotDir, 'T14-betslip-bet-amount-input-multi.png', testInfo);
  });
  ...
  test('T28-Verify functionality of "Login" button on betslip for single and multi tab', async ({ betslipPage, page }, testInfo) => {
    await OddsSelection(1, page);
    ...
    await betslipPage.verifyLoginBtnBetslip();
    await betslipPage.clickLoginBtnBetslip();
    await ScreenshotHelper(page, screenshotDir, 'T28-betslip-login-after-click-multi.png', testInfo);
  });
