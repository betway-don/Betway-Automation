// npx playwright test src/regions/GH/tests/smoke/feeds/feeds.spec.ts --config=playwright.GH.config.ts --headed
...
test.describe('Feeds Tests', () => {
    ...
    test('T5-Verify presence and functionality of feed section on betway feeds section', async ({ feedsPage }, testInfo) => {
        ...
        await feedsPage.captureScreenshot('firstFeeds', screenshotDir, 'T5-Feeds', testInfo);
    });
